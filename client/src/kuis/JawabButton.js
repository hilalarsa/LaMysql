...
/**Button ini ada di komponen kerjakanKuis, untuk menerima inputField dari kerjakanKuis, nilainya dari props
...
class JawabButton extends Component {
    ...
    submitJawaban(){
        const {
            ...
            attempt,
            id_soal
        } = this.props; //ambil dari props komponen di kerjakanKuis
        let self = this;
        self.setState({
            attempt: attempt
        })
        ...
        fetch(process.env.REACT_APP_SERVER_ENDPOINT + "/kuis/jawabKuis", {
            method: 'POST',
            headers: {
            ...
            if (response.status >= 400) {
                ...
                return response.json().then(function (data) {
                    if (data.Error) {
                        ...
                        self.setState({
                            hasil: "Jawaban benar, Attempt final: " + self.state.attempt
                        })
            ...
            fetch(process.env.REACT_APP_SERVER_ENDPOINT + "/kuis/simpanJawaban", {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': localStorage.getItem('token')
